...
<div class="card">
    ...
    <div class="card-body">
        ...
        @endif
        <form action="{{ route('obwis.update', $obwis->id) }}" method="POST" enctype="multipart/form-data">
        @method('PATCH')
        @csrf
            <div class="mb-3">
                <label class="form-label">ID Obwis:</label>
                <div class="form-control-plaintext">{{ $obwis->obwis_id }}</div> <!-- Display as plain text -->
                <input type="hidden" name="obwis_id" value="{{ $obwis->obwis_id }}"> <!-- Keep hidden input for form submission -->
            </div>
            <div class="mb-3">
            ...
            <div class="mb-4">
                ...
                <div class="form-check">
                    <input type="checkbox" name="increment_status" id="increment_status" class="form-check-input" value="1" {{ old('increment_status', $obwis->status) ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="cp" class="form-control" placeholder="CP (Contact Person)" value="{{ old('cp', $obwis->cp) }}" readonly> <!-- Make read-only -->
                @error('cp')
                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <textarea name="alamat" class="form-control" placeholder="Alamat">{{ old('alamat', $obwis->alamat) }}</textarea>
                @error('alamat')
                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <div class="input-group">
                    <input type="url" id="maps" name="maps" class="form-control" placeholder="Google Maps Link" value="{{ old('maps', $obwis->maps) }}">
            ...
            <div class="mb-3">
                ...
                <input type="url" id="gambar" name="gambar" class="form-control" placeholder="Gambar URL" value="{{ old('gambar', $obwis->gambar) }}">
                @error('gambar')
                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                @enderror
